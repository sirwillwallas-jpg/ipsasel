-- Migration: Drop and recreate full schema (DANGEROUS - DATA LOSS)
-- WARNING: This script will DROP existing tables and types and then recreate them
-- ONLY RUN IF YOU HAVE A BACKUP OR IF YOU ACCEPT DATA LOSS

BEGIN;

-- Drop tables (order not important because of CASCADE)
...
-- Recreate schema (based on schema.sql)
